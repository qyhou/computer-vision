...
class ResNet(nn.Module):
    def __init__(self, block, layers, classes=1000, in_channels=3, channel_ratio=1.0):
        ...
        self.conv_2, self.in_channels = self._make_layer(block, self.in_channels, make_divisible(64 * channel_ratio),
                                                         layers[0])
        self.conv_3, self.in_channels = self._make_layer(block, self.in_channels, make_divisible(128 * channel_ratio),
                                                         layers[1], stride=2)
        self.conv_4, self.in_channels = self._make_layer(block, self.in_channels, make_divisible(256 * channel_ratio),
                                                         layers[2], stride=2)
        self.conv_5, self.in_channels = self._make_layer(block, self.in_channels, make_divisible(512 * channel_ratio),
                                                         layers[3], stride=2)

        self.adapt_avg_pooling = nn.AdaptiveAvgPool2d(1)
    ...
    def _make_layer(block, in_channels, out_channels, blocks, stride=1):
        ...
        layers = [block(in_channels, out_channels, stride, down_sampling)]
